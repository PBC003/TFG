import {
  ...
  isEmptyAnswer,
  serializeCsvValue,
} from '../../../src/quizzes/quiz-analytics.service';
...
describe('QuizAnalyticsService', () => {
  ...
  it('returns zeroed analytics and fallback participant names when there are no completed attempts', async () => {
    const { service, quizAttemptModel, userRepository, quizzesSharedService } =
      createService();

    quizzesSharedService.findManagedQuizDocumentOrThrow.mockResolvedValue(quiz);
    quizAttemptModel.find.mockReturnValueOnce(createExecChain([]));
    userRepository.findBy.mockResolvedValueOnce([]);
    ...
    expect(analytics.summary).toEqual(
      expect.objectContaining({
        ...
        averageScoreOverTen: 0,
        bestScoreOverTen: 0,
        worstScoreOverTen: 0,
      }),
    );
    expect(
      analytics.scoreDistribution.every((bucket) => bucket.count === 0),
    ).toBe(true);
    expect(analytics.questionStats).toEqual([]);
    ...
    const analyticsWithFallback = await service.getQuizAnalytics('quiz-1', {
      id: 7,
      role: 'TEACHER',
    } as never);
    expect(analyticsWithFallback.attempts[0]).toEqual(
      expect.objectContaining({
  ...
  it('lists empty history and falls back to raw participant names in attempt details', async () => {
    const {
      service,
      quizModel,
      quizAttemptModel,
    ...
    quizAttemptModel.find.mockReturnValueOnce(createExecChain([]));
    quizModel.find.mockReturnValueOnce(createExecChain([]));
    await expect(service.listHistoryForUser(77)).resolves.toEqual([]);
    ...
    const detail = await service.getAttemptDetail(
      'quiz-1',
      'attempt-fallback',
      {
        id: 7,
    ...
    expect(detail.participantDisplayName).toBe('Guest User');
    expect(detail.scoreOverTen).toBe(0);
  });

  it('computes question stats for empty strings, empty arrays and preserves question ordering', async () => {
    const { service, quizAttemptModel, userRepository, quizzesSharedService } =
      createService();
      ...
      createExecChain([
        {
          ...
          maxPoints: 3,
          questions: [
            { ...baseQuestion, questionId: 'q-2', order: 2, points: 2 },
            { ...baseQuestion, questionId: 'q-1', order: 1, points: 1 },
          ],
          answers: [
            { questionId: 'q-2', value: '', earnedPoints: 0, isCorrect: false },
            { questionId: 'q-1', value: [], earnedPoints: 1, isCorrect: false },
          ],
          save: jest.fn(),
        },
      ]),
    ...
    expect(analytics.questionStats.map((item) => item.questionId)).toEqual([
      'q-1',
      'q-2',
    ]);
    expect(analytics.questionStats).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ questionId: 'q-1', unansweredCount: 1 }),
        expect.objectContaining({ questionId: 'q-2', unansweredCount: 1 }),
      ]),
    );
  });

  it('escapes quoted quiz titles in csv exports', async () => {
    const { service, quizAttemptModel, userRepository, quizzesSharedService } =
      createService();

    quizzesSharedService.findManagedQuizDocumentOrThrow.mockResolvedValue({
      ...quiz,
      title: 'Quiz; "Especial"',
    });
    quizAttemptModel.find.mockReturnValueOnce(
    ...
    expect(csv).toContain('"Quiz; ""Especial"""');
  });
  it('exposes helper behavior for analytics calculations and csv serialization', () => {
    expect(computeScoreOverTen(0, 0)).toBe(0);
    expect(computeScoreOverTen(2, 4)).toBe(5);
    ...
    expect(serializeCsvValue('plain')).toBe('plain');
    expect(serializeCsvValue(3)).toBe('3');
    expect(serializeCsvValue(true)).toBe('true');
    expect(serializeCsvValue(4n)).toBe('4');
    expect(serializeCsvValue(new Date('2026-04-13T11:00:00.000Z'))).toBe(
      '2026-04-13T11:00:00.000Z',
    );
    expect(serializeCsvValue({ raw: 'payload' })).toBe('{"raw":"payload"}');

    expect(escapeCsvField('line\nfeed')).toBe('line feed');
    expect(escapeCsvField('needs;escaping')).toBe('"needs;escaping"');
