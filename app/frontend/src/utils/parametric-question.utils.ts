...
export const DEFAULT_PARAMETRIC_TOLERANCE = 0.01;
...
export function resolveParametricTolerance(
  ...
  return Number((config.tolerance ?? DEFAULT_PARAMETRIC_TOLERANCE).toFixed(6));
}
...
export function getParametricTemplateVariantCount(
  ...
  switch (templateId) {
    ...
    case "limit_logarithmic":
    case "integral_logarithmic":
      return A_VARIANTS.length;
    case "integral_inverse_quadratic":
      return INVERSE_QUADRATIC_ALLOWED_A_VALUES.length;
    case "series_geometric":
      return I_VARIANTS.length;
  }
}

function buildSampleStatement(
  templateId: ParametricTemplateId,
): Pick<ParametricSampleInstance, "statement" | "correctAnswerLatex"> {
  switch (templateId) {
    case "limit_trigonometric": {
      const a = randomPick(A_VARIANTS);
      const b = a + 1;
...
export function generateParametricSampleInstance(
  ...
  const sample = buildSampleStatement(config.templateId);
